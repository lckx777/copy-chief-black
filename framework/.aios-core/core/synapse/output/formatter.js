...
function formatStarCommands(result) {
  const meta = result.metadata || {};
  const command = (Array.isArray(meta.commands) ? meta.commands.join(', ') : meta.command) || meta.source || 'unknown';
  const lines = [
    '[STAR-COMMANDS]',
    '============================================================',
    `[*${command}] COMMAND:`,
...
function formatSummary(results, _metrics) {
  ...
  for (const result of results) {
    ...
    const meta = result.metadata || {};
    // L6 keyword results use matchedDomains; L7 star-command uses commands
    const source = (
      meta.source ||
      meta.domain ||
      (Array.isArray(meta.matchedDomains) ? meta.matchedDomains.join('+') : null) ||
      (Array.isArray(meta.commands) ? meta.commands.join('+') : null) ||
      'unknown'
    ).toUpperCase();
